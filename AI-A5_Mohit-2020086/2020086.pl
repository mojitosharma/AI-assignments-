...
setting_env:-                             % helper funtion used to set all facts and rules 
    ...
    retractall(ssh_selected(_)),
    retractall(done_course(_)),
...
define_course:-  
    ...
    assert(course(history,'Nation and her Narratives',ssh212,2)),
...
read_file_l(InputStream,[]) :-
    at_end_of_stream(InputStream).

read_file_l(InputStream,[H|T]) :-
    \+ at_end_of_stream(InputStream),
    read(InputStream,H),
    read_file_l(InputStream,T).

define_fact([H|T]):-
    define_fact(T);assert(H).
define_fact([]).

read_input:-
    open('input.txt', read, InputStream),
    read_file_l(InputStream,List),
    close(InputStream),
    define_fact(List).

student_info:-
    name(Name),
    write('Hi '),write(Name),write('!, Welcome, to Electives advisory / prediction system.'),nl.

career_choice:-
    write('------------------------------------------------------------'),nl,
    write('Carrer Option You Selected :  '),
    career_selected(X),career(_,X,Y),write(Y),nl.
...
interest_course:-
    findall(X, ssh_selected(X), List),
    nl,write('------------------------------------------------------------'),nl,
    write('Area of Interest Choosen :'),nl,
    forall(ssh_selected(X),(ssh(X,Y),write(Y),write(', '))),
    write('.'),nl,
    helper1(List).
...
pre_req_done(List):-
    \+ career_selected(na),
    write('------------------------------------------------------------'),nl,
    write('Pre-requisites Done: '),nl,
    findall(X, done_course(X), List),
    forall(done_course(X),(course(A,B,X,D),write(X),write(', '),write(B),nl)),
    helper2,
    helper4(List),fail;true.
